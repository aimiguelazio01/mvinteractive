...
2. Run the `send_to.js` script from the `archviz_email2` directory.
...
- Sends via Resend API from `MVs Archviz <hello@mvirgilstudio.com>` using the selected API key
...
| Subject PT | Maquetes Interativas para Apresentações de Arquitetónicas                          |
