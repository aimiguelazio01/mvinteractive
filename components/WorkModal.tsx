...
const WorkModal: React.FC<WorkModalProps> = ({ work, onClose, lang }) => {
    ...
    return (
        <div
            ref={overlayRef}
            className="fixed inset-0 z-[2000] flex items-center justify-center p-4 md:p-12 bg-black/95 backdrop-blur-2xl overflow-hidden"
            onClick={handleClose}
        >
            <div
                ref={modalContainerRef}
                className="relative w-full max-w-7xl h-full max-h-[85vh] bg-[#050505] border border-white/10 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden shadow-[0_0_100px_rgba(255,255,255,0.05)] z-10 custom-scrollbar"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Close Interaction */}
                <button
                    onClick={handleClose}
                    className="absolute top-4 right-4 md:top-8 md:right-8 z-[60] group flex items-center gap-4 text-gray-800/50 hover:text-gray-800 transition-colors duration-500"
                >
                ...
                {/* Left Side: Media */}
                <div ref={mediaContainerRef} className="w-full md:w-[60%] aspect-video md:h-full relative bg-black overflow-hidden shrink-0">
                ...
                <div ref={contentRef} className="w-full md:w-[40%] p-8 md:p-16 flex flex-col justify-start md:justify-center bg-[#050505] relative border-l border-white/5">
                    ...
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold text-gray-800 mb-8 uppercase tracking-tighter leading-tight break-words">
                        {t.worksIndex[activeWork.id as keyof typeof t.worksIndex]?.title || activeWork.title}
                    </h2>

                    <div className="w-16 h-[2px] bg-white/20 mb-12" />

                    <p className="text-sm md:text-base text-concrete/70 leading-relaxed font-light mb-12 max-w-sm tracking-wide">
                    ...
                    {activeWork.externalLink && (
                        ...
                        >
                            <span className="relative z-10 text-[11px] font-bold tracking-[0.3em] text-gray-800/70 group-hover/btn:text-accent transition-colors duration-500 uppercase">
                                {t.ui.visitProject}
                            </span>
                            <span className="relative z-10 text-gray-800/30 group-hover/btn:text-accent transition-all duration-500 transform group-hover/btn:translate-x-1">→</span>
                    ...
                    <div className="space-y-6 text-gray-800/60">
