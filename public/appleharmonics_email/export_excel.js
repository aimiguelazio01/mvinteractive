...
export async function logEmailToExcel(emailData) {
  const excelPath = path.join(__dirname, 'sent_appleharmonics_emails.xlsx');
  let workbook = new ExcelJS.Workbook();
  let worksheet;

  if (fs.existsSync(excelPath)) {
    await workbook.xlsx.readFile(excelPath);
    worksheet = workbook.getWorksheet('Sent Apple Harmonics Emails') || workbook.addWorksheet('Sent Apple Harmonics Emails');
  } else {
    worksheet = workbook.addWorksheet('Sent Apple Harmonics Emails');
    worksheet.columns = [
  ...
  if (!exists) {
    worksheet.addRow({
      ...
      from: emailData.from || 'MVs Apple Harmonics <hello@mvirgilstudio.com>',
