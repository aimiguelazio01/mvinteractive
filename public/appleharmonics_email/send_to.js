/**
 * MVs Apple Harmonics — CLI Email Sender
 * 
 * Usage:
...
async function sendEmail() {
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    try {
      ...
      if (res.error && (res.error.statusCode === 403 || res.error.message?.includes('domain is not verified'))) {
      ...
      if (error) {
        ...
        try {
          ...
          console.log(`📊 Logged to sent_appleharmonics_emails.xlsx`);
        } catch (excelErr) {
